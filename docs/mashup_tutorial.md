...
This degree of separation enables you to integrate different external libraries into your application without the need to stitch them together.
...
The mentiond external libraries are:
...
* [Flow and Places](https://github.com/LaxarJS/laxar/blob/master/docs/manuals/flow_and_places.md)
...
times in [mashup_demo.json](../application/pages/mashup_demo.json) as is the case with the ChartWidget.
...
In our context data sets consist of a set of time series to be edited and visualized by other widgets:
...
Other widgets in this setup are only allowed to consume and/or update the resource.
...
They act as slave widgets in our setup. You may have guessed that the corresponding subscription looks like this:
...
This one-liner does even more: It looks up the attribute `resource` of feature `timeSeries` and subscribes to didReplace events.
When such an event is received the data is set as attribute `timeSeries` of `$scope.resources`.
The above snippet also subscribes to didUpdate events, evaluates the patches attached to them and integrates them into the existing resource.
There's even more: you may immediately call a function once the replaced or updated resource is received.
The subscription in TableEditorWidget is initialized as follows:
...
We call `applyPatches()` when the existing resource is updated and `convertToChartModel()` when the entire resource has been replaced.
The idea behind it is simple:
we want to patch only the values that have been changed so we can see a nice transition from the old to the new value in the charts.
...
What we do then is to compute the difference between the old resource representation and the modified one.
...
The ChartWidget subscripes to didReplace and didUpdate events.
The good news is we can actually treat them alike with:

```javascript
patterns.resources.handlerFor( $scope ).registerResourceFromFeature( 'timeSeries' );
```

If the is updated the above snippet ensures that the accompanying patches are evaluated and integrated into the existing resource, which is kept in attribute `timeSeries` of `$scope.resources`.
...
This approach results in a nice transition from the old values to the new values while that chart is not redrawn.


## Including External Libraries ##
In a LaxarJS application we manage external libraries with *[RequireJS](http://requirejs.org/)*, *[npm](http://www.npmjs.org)* and *[Bower](http://bower.io/)*.
...
The ChartWidget expects a resource with one or multiple time series and displays them in a chart.
For the visualization of a chart we use the library *Angular-nvD3*.
To download the library and its dependencies we use bower. When downloading there is a dependency conflict between Angular-nvD3 and NVD3 and their common dependency D3.
We can resolve it manually in [bower.json](../bower.json#L22).
...
When the dependency 'angular-nvd3' is mentioned, requireJS will be able to build the dependency tree from our shim config.
A valid order of loading the above dependencies is ('d3', 'nvd3', 'angular-nvd3').
